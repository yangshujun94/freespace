...
int PARKINGLOT::_encodeNoHash(void *buf, int offset, int maxlen) const
{
    ...
    tlen = __byte_encode_array(buf, offset + pos, maxlen - pos, &this->apaParklotTypeAndDir, 1);
    if(tlen < 0) return tlen; else pos += tlen;
...
int PARKINGLOT::_decodeNoHash(const void *buf, int offset, int maxlen)
{
    ...
    tlen = __byte_decode_array(buf, offset + pos, maxlen - pos, &this->apaParklotTypeAndDir, 1);
    if(tlen < 0) return tlen; else pos += tlen;
...
uint64_t PARKINGLOT::_computeHash(const __lcm_hash_ptr *p)
{
    ...
    uint64_t hash = 0x0d6899d854f454bbLL +
