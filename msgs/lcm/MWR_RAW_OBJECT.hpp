...
#ifndef __MWR_RAW_OBJECT_hpp__
#define __MWR_RAW_OBJECT_hpp__

#include "HEADER.hpp"


class MWR_RAW_OBJECT
{
    public:
        HEADER     stHeader;
...
int MWR_RAW_OBJECT::encode(void *buf, int offset, int maxlen) const
{
    int pos = 0, tlen;
...
int MWR_RAW_OBJECT::decode(const void *buf, int offset, int maxlen)
{
    int pos = 0, thislen;
...
int MWR_RAW_OBJECT::getEncodedSize() const
{
    return 8 + _getEncodedSizeNoHash();
}

int64_t MWR_RAW_OBJECT::getHash()
{
    static int64_t hash = _computeHash(NULL);
    return hash;
}

const char* MWR_RAW_OBJECT::getTypeName()
{
    return "MWR_RAW_OBJECT";
}

int MWR_RAW_OBJECT::_encodeNoHash(void *buf, int offset, int maxlen) const
{
    int pos = 0, tlen;
    ...
    tlen = __float_encode_array(buf, offset + pos, maxlen - pos, &this->fwObstacle, 1);
    if(tlen < 0) return tlen; else pos += tlen;

    tlen = __float_encode_array(buf, offset + pos, maxlen - pos, &this->fwExist, 1);
    if(tlen < 0) return tlen; else pos += tlen;

    return pos;
}

int MWR_RAW_OBJECT::_decodeNoHash(const void *buf, int offset, int maxlen)
{
    int pos = 0, tlen;
    ...
    tlen = __float_decode_array(buf, offset + pos, maxlen - pos, &this->fwObstacle, 1);
    if(tlen < 0) return tlen; else pos += tlen;

    tlen = __float_decode_array(buf, offset + pos, maxlen - pos, &this->fwExist, 1);
    if(tlen < 0) return tlen; else pos += tlen;

    return pos;
}

int MWR_RAW_OBJECT::_getEncodedSizeNoHash() const
{
    int enc_size = 0;
...
uint64_t MWR_RAW_OBJECT::_computeHash(const __lcm_hash_ptr *p)
{
    const __lcm_hash_ptr *fp;
    for(fp = p; fp != NULL; fp = fp->parent)
        if(fp->v == MWR_RAW_OBJECT::getHash)
            return 0;
    const __lcm_hash_ptr cp = { p, (void*)MWR_RAW_OBJECT::getHash };

    uint64_t hash = 0xccdbf4eb07b61dcfLL +
