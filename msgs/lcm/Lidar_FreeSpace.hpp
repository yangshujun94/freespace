...
#ifndef __Lidar_FreeSpace_hpp__
#define __Lidar_FreeSpace_hpp__

#include "HEADER.hpp"
#include <vector>
#include "Lidar_FreeSpace_Point.hpp"


class Lidar_FreeSpace
{
    public:
        HEADER     stHeader;
        ...
        std::vector< Lidar_FreeSpace_Point > gstPoints;
...
int Lidar_FreeSpace::encode(void *buf, int offset, int maxlen) const
{
    int pos = 0, tlen;
...
int Lidar_FreeSpace::decode(const void *buf, int offset, int maxlen)
{
    int pos = 0, thislen;
...
int Lidar_FreeSpace::getEncodedSize() const
{
    return 8 + _getEncodedSizeNoHash();
}

int64_t Lidar_FreeSpace::getHash()
{
    static int64_t hash = _computeHash(NULL);
    return hash;
}

const char* Lidar_FreeSpace::getTypeName()
{
    return "Lidar_FreeSpace";
}

int Lidar_FreeSpace::_encodeNoHash(void *buf, int offset, int maxlen) const
{
    int pos = 0, tlen;
...
int Lidar_FreeSpace::_decodeNoHash(const void *buf, int offset, int maxlen)
{
    int pos = 0, tlen;
...
int Lidar_FreeSpace::_getEncodedSizeNoHash() const
{
    int enc_size = 0;
...
uint64_t Lidar_FreeSpace::_computeHash(const __lcm_hash_ptr *p)
{
    const __lcm_hash_ptr *fp;
    for(fp = p; fp != NULL; fp = fp->parent)
        if(fp->v == Lidar_FreeSpace::getHash)
            return 0;
    const __lcm_hash_ptr cp = { p, (void*)Lidar_FreeSpace::getHash };

    uint64_t hash = 0xb417ad9b66c3d267LL +
         HEADER::_computeHash(&cp) +
         Lidar_FreeSpace_Point::_computeHash(&cp);
