...
export LD_LIBRARY_PATH=:/mnt/pegasus/YSG_Version/lib:/mnt/pegasus/AIV_iecu2.0_Release/01_Application/02_lib:/opt/uto/other/uto_third:$LD_LIBRARY_PATH
export ROS_LOG_DIR=/opt/uto/log/
...
log_file="/opt/uto/log/run_fs_$timestamp.log"
echo "$timestamp" >>$log_file

if [ ! -f app_fs ]; then
  echo "Error: cannot access 'app_fs': No such file or directory"
  echo "Error: cannot access 'app_fs': No such file or directory" >>$log_file
  echo "mv run_fs.sh to 'app_fs' file's dir" >>$log_file
  exit 1
fi

PROJECT_NAME=$(cat /mnt/pegasus/aiv_project_info.json | python3 -c "import json; import sys; obj=json.load(sys.stdin); print (obj['project_name'])")
...
while [ 1 ]; do
  ...
  if [[ $pid_no == "" ]]; then
    ...
    nohup ./app_fs --ros-args --log-level WARN -p project_name:=$PROJECT_NAME >/dev/null 2>&1 &
